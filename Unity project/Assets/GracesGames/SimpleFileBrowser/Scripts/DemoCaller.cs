...
namespace GracesGames.SimpleFileBrowser.Scripts {
	...
	public class DemoCaller : MonoBehaviour {
		...
		// where do we put the content of the loaded file
		public ElectrodesData setsStorage;
		public UiController uiStorage;
		public SavesManager savesManager;
		public GameObject datasetsPanel;
		public GameObject colormapsPanel;
...
////////////////////////////////////////////////////////////////////////////////////////////
		...
		private void LoadFileUsingPath(string path) {
			if (path.Length != 0) {
				...
				if(datasetsPanel.activeInHierarchy) {
					string readText = File.ReadAllText(path);
					setsStorage.data = readText;
					//storage.ExtractNInstantiate();
					setsStorage.CreateDataset(path);
				} else if (colormapsPanel.activeInHierarchy) {
					string readText = File.ReadAllText(path);
					uiStorage.AddColormap(readText);
				} 
				// sinon, on est dans le cas du chargement d'un fichier de sauvegarde
				else {
					BinaryFormatter bFormatter = new BinaryFormatter();
					...
					savesManager.UnpackData(fileData);
